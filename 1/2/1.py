...
while True:
    ...
    if ret:
        ...
        if red_car_lane:
            if safe_lanes:
                if red_car_lane[0] + 1 in safe_lanes:
                    print("Safe lane is on the right of the current lane of the red car")
                    execute_command("right")
                elif red_car_lane[0] - 1 in safe_lanes:
                    print("Safe lane is on the left of the current lane of the red car")
                    execute_command("left")
                elif red_car_lane[0] in safe_lanes:
                    print("Safe lane is in the current lane of the red car")
            elif blue_car_lane or yellow_car_lane:
                if red_car_lane[0] + 1 in lanes.keys():
                    print("Both adjacent lanes are safe, but current lane is not. Choosing right by default.")
                    execute_command("right")
