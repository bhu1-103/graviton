...
void move_right() {
    ...
    right_shift_count++;
    if (right_shift_count >= 3) {
        in_rightmost_lane = true;
    }
}
...
void process_objects(cv::Mat& frame, const std::vector<std::vector<cv::Point>>& contours_blue, 
    ...
    if (!in_rightmost_lane) {
        ...
        if (!distance_yellow.empty() && *std::min_element(distance_yellow.begin(), distance_yellow.end()) < THRESHOLD_DISTANCE) {
            move_left();  
        }
    }
}
