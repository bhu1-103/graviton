...
void move_left() {
    ...
    left_shift_counter++;
    right_shift_counter--; // Reset right shift counter when moving left
}
...
void process_objects(cv::Mat& frame, const std::vector<std::vector<cv::Point>>& contours_blue,
    ...
    const int num_lanes = 9;
    const int lane_width = frame.cols / num_lanes;
    const int lane_start = lane_width; // Start from the second lane
    const int lane_end = (num_lanes - 2) * lane_width; // End before the last lane
    const int player_x = lane_width * 4 + lane_width / 2; // Initial position in the middle of the road
    ...
    for (int lane = 1; lane < num_lanes - 1; ++lane) {
        if (lane_occupied[lane]) {
            std::cout << "Lane " << lane << " occupied" << std::endl;
        }
    }
